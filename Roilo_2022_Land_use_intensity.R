###################################################
#
# Title: Roilo_2022_Land_use_intensity.R
# Purpose: calculate different land use intensity metrics in different grids over the Mulde
...
# Date: last updated on October 27th, 2022.
#
###################################################
...
# remove unclassified LC area at southern border of the Mulde
lucf = lucf[lucf$DN>0,]
## now calculate the centroid (or, if falling outside the polygon, a random innner point) of each polygon 
...
## voronoi polygons are now computed for a much larger area; cut them to the Mulde extent
...
###### COMPUTATION OF LAND USE VARIABLES  --------------------------
# now, compute the land use intensity variables for each of the three grids
...
# load IACS data for the year 2019
...
# calculate area of each field parcel and then its centroid
...
# compute a general LUI indicator, based on the kg N input (irrespectively of arable/grassland cover)
...
names(squ) = c("FID", "UAA","Arable","Arable_UAA", "Field_size", "N_input","Intens_farm", "Conv_farm" , "ALU_homog" , "LCLU_homog", "LC_homog")
names(hex) = c("FID", "UAA","Arable","Arable_UAA", "Field_size", "N_input","Intens_farm", "Conv_farm" , "ALU_homog" , "LCLU_homog", "LC_homog")
names(voro) = c("FID", "UAA","Arable","Arable_UAA", "Field_size", "N_input","Intens_farm", "Conv_farm" , "ALU_homog" , "LCLU_homog", "LC_homog")
...
library(corrplot)
## square grid 
cmat <- cor(squ[,2:11], method="spearman")
testRes = cor.mtest(cmat, conf.level = 0.95)
# save image of correlation matrix with the correlation coefficients
corrplot(cmat, order = 'original', method="color", addCoef.col = 'black', tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),
         title= "Spearman correlation matrix - square grid", mar=c(2,2,2,2)) 
# save image of correlation matrix with the significance levels
corrplot(cmat, order = 'original',  method="color", p.mat = testRes$p, sig.level = c(0.001, 0.01, 0.05), pch.cex = 1.5, insig = 'label_sig', pch.col = 'black',
         tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),title= "Spearman correlation matrix - square grid", mar=c(2,2,2,2)) 
# repeat with other type of correlation method - pearson
cmat <- cor(squ[,2:11], method="pearson")
testRes = cor.mtest(cmat, conf.level = 0.95)
corrplot(cmat, order = 'original', method="color", addCoef.col = 'black', tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'), 
         title= "Pearson correlation matrix - square grid", mar=c(2,2,2,2)) # save image with and without the significance levels
corrplot(cmat, order = 'original', method="color", p.mat = testRes$p, sig.level = c(0.001, 0.01, 0.05), pch.cex = 1.5, insig = 'label_sig', pch.col = 'black',
         tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),title= "Pearson correlation matrix - square grid", mar=c(2,2,2,2)) 
# repeat with kendall correlation
cmat <- cor(squ[,2:11], method="kendall")
testRes = cor.mtest(cmat, conf.level = 0.95)
corrplot(cmat, order = 'original', method="color", addCoef.col = 'black', tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'), 
         title= "Kendall correlation matrix - square grid", mar=c(2,2,2,2)) # save image with and without the significance levels
corrplot(cmat, order = 'original', method="color", p.mat = testRes$p, sig.level = c(0.001, 0.01, 0.05), pch.cex = 1.5, insig = 'label_sig', pch.col = 'black',
         tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),title= "Kendall correlation matrix - square grid", mar=c(2,2,2,2)) 

## hexagonal grid
cmat <- cor(hex[,2:11], method="spearman")
testRes = cor.mtest(cmat, conf.level = 0.95)
corrplot(cmat, order = 'original', method="color", addCoef.col = 'black', tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),
         title= "Spearman correlation matrix - hexagonal grid", mar=c(2,2,2,2)) # save image with and without the significance levels
corrplot(cmat, order = 'original',  method="color", p.mat = testRes$p, sig.level = c(0.001, 0.01, 0.05), pch.cex = 1.5, insig = 'label_sig', pch.col = 'black',
         tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),title= "Spearman correlation matrix - hexagonal grid", mar=c(2,2,2,2)) 
# repeat with other type of correlation method - pearson
cmat <- cor(hex[,2:11], method="pearson")
testRes = cor.mtest(cmat, conf.level = 0.95)
corrplot(cmat, order = 'original', method="color", addCoef.col = 'black', tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'), 
         title= "Pearson correlation matrix - hexagonal grid", mar=c(2,2,2,2)) # save image with and without the significance levels
corrplot(cmat, order = 'original', method="color", p.mat = testRes$p, sig.level = c(0.001, 0.01, 0.05), pch.cex = 1.5, insig = 'label_sig', pch.col = 'black',
         tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),title= "Pearson correlation matrix - hexagonal grid", mar=c(2,2,2,2)) 
# repeat with kendall correlation
cmat <- cor(hex[,2:11], method="kendall")
testRes = cor.mtest(cmat, conf.level = 0.95)
corrplot(cmat, order = 'original', method="color", addCoef.col = 'black', tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'), 
         title= "Kendall correlation matrix - hexagonal grid", mar=c(2,2,2,2)) # save image with and without the significance levels
corrplot(cmat, order = 'original', method="color", p.mat = testRes$p, sig.level = c(0.001, 0.01, 0.05), pch.cex = 1.5, insig = 'label_sig', pch.col = 'black',
         tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),title= "Kendall correlation matrix - hexagonal grid", mar=c(2,2,2,2)) 
## voronoi grid
cmat <- cor(voro[,2:11], method="spearman")
testRes = cor.mtest(cmat, conf.level = 0.95)
corrplot(cmat, order = 'original', method="color", addCoef.col = 'black', tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),
         title= "Spearman correlation matrix - voronoi", mar=c(2,2,2,2)) # save image with and without the significance levels
corrplot(cmat, order = 'original',  method="color", p.mat = testRes$p, sig.level = c(0.001, 0.01, 0.05), pch.cex = 1.5, insig = 'label_sig', pch.col = 'black',
         tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),title= "Spearman correlation matrix - voronoi", mar=c(2,2,2,2)) 
# repeat with other type of correlation method - pearson
cmat <- cor(voro[,2:11], method="pearson")
testRes = cor.mtest(cmat, conf.level = 0.95)
corrplot(cmat, order = 'original', method="color", addCoef.col = 'black', tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'), 
         title= "Pearson correlation matrix - voronoi", mar=c(2,2,2,2)) # save image with and without the significance levels
corrplot(cmat, order = 'original', method="color", p.mat = testRes$p, sig.level = c(0.001, 0.01, 0.05), pch.cex = 1.5, insig = 'label_sig', pch.col = 'black',
         tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),title= "Pearson correlation matrix - voronoi", mar=c(2,2,2,2)) 
# repeat with kendall correlation
cmat <- cor(voro[,2:11], method="kendall")
testRes = cor.mtest(cmat, conf.level = 0.95)
corrplot(cmat, order = 'original', method="color", addCoef.col = 'black', tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'), 
         title= "Kendall correlation matrix - voronoi", mar=c(2,2,2,2)) # save image with and without the significance levels
corrplot(cmat, order = 'original', method="color", p.mat = testRes$p, sig.level = c(0.001, 0.01, 0.05), pch.cex = 1.5, insig = 'label_sig', pch.col = 'black',
         tl.pos = 'd', type="upper",tl.cex=0.8,  col = COL2('PiYG'),title= "Kendall correlation matrix - voronoi", mar=c(2,2,2,2)) 


### DENSITY DISTRIBUTIONS / RIDGEPLOTS  -------------------------------------------------------------
# construct a 2 km buffer around the study area boundary to mask out points too close to the border
buf = st_buffer(mulde, -2000)
# select centroids of square grid 
pts = st_centroid(sqgrid) 
pts = pts[which(st_intersects(pts, buf, sparse=F) == T),]
# save to file
st_write(pts, "Land use intensity/points/4777_SQGrid_centroids.shp")
# check spatial output and distribution across the study area
mapview(buf) + pts

# extract values from the three grids
# first, give one unique number to each point
pts$np = 1:nrow(pts); pts = pts[,c("np", "geometry")]
ptss = st_join(pts, sqgrid[,2:11]) 
ptsh = st_join(pts, hexgrid[,2:11])
# many more points in the intersection with the hexagonal grid because lots of them fall on the border between two cells; remove duplicates
ptsh = ptsh[-which(duplicated(ptsh$np)),]
ptsv = st_join(pts, vor[,2:11])
allpts = rbind(st_drop_geometry(ptss),  st_drop_geometry(ptsh),  st_drop_geometry(ptsv))
allpts$grid = c(rep("Square", times=nrow(ptss)), rep("Hexagon", times=nrow(ptsh)),rep("Voronoi", times=nrow(ptsv)))
allpts$grid = as.factor(allpts$grid)

# plot density distribution of values for each metric
...
# plot them only for the 4777 extracted points
# first, reorder the variables
allpts2 = gather(allpts, key = Metric, Intensity, UAA:LC_homog)
allpts2 <- allpts2 %>%  mutate(Metric=factor(Metric,levels=rev(unique(allpts2$Metric))) )
allpts2 <- allpts2 %>%  mutate(grid=factor(grid,levels=unique(allpts2$grid)) )
# add extra columns for visualization of only certain points (same points across grids!)
allpts2$shape = 0; allpts2$shape[allpts2$np == 876] <-1; allpts2$shape[allpts2$np == 2696] <-2; allpts2$shape[allpts2$np == 2242] <-3
allpts2$color = ifelse(allpts2$shape>0,1,0)

# plot ridgeplots for all grids, one panel per grid
ggplot(allpts2, aes(x=Intensity, y=Metric, fill = factor(stat(quantile)))) +
  stat_density_ridges(
    geom = "density_ridges_gradient", calc_ecdf = TRUE,#scale=0.9    
    quantiles = 5, quantile_lines = TRUE, jittered_points = TRUE, aes(point_alpha=color, point_shape=shape, point_stroke = 1.5, point_color="dark_red")
...
##### KULLBACK-LEIBLER DISTANCE, calculated across distributions ------------
pts = st_read("Land use intensity/points/4777_SQGrid_centroids.shp")
pts$np = 1:nrow(pts); pts = pts[,c("np", "geometry")]
ptss = st_join(pts, sqgrid[,2:11]) 
ptsh = st_join(pts, hexgrid[,2:11]); ptsh = ptsh[-which(duplicated(ptsh$np)),]
ptsv = st_join(pts, vor[,2:11])

# kullback_leibler_distance
library(philentropy)
# create function to compute symmetrical kullback_leibler_distance for each pair of distributions
...
sym_KLD(ptss$UAA, ptsh$UAA, unit="log", epsilon = 0.00001)
# now, put all metrics in the same dataframe and calculate KLD for each pair
allmets = cbind(st_drop_geometry(ptss[,2:11]), st_drop_geometry(ptsh[,2:11]), st_drop_geometry(ptsv[,2:11]))
names(allmets) <- c("UAA_SQ","Arable_SQ", "Arable_UAA_SQ", "Field_size_SQ", "N_input_SQ", "Intens_f_SQ", "Conv_farm_SQ", "ALU_homog_SQ", "LULC_homog_SQ", "LC_homog_SQ",
                    "UAA_HX","Arable_HX", "Arable_UAA_HX", "Field_size_HX", "N_input_HX", "Intens_f_HX", "Conv_farm_HX", "ALU_homog_HX", "LULC_homog_HX", "LC_homog_HX",
                    "UAA_VO","Arable_VO", "Arable_UAA_VO", "Field_size_VO", "N_input_VO", "Intens_f_VO", "Conv_farm_VO", "ALU_homog_VO", "LULC_homog_VO", "LC_homog_VO")
...
write.table(kld_df, sep=";", dec=",", row.names = T, "Land use intensity/Kullback_leibler_divergence_20220922.csv")
